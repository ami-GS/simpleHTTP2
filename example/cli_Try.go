...
func main() {
	...
	client.Send(http2.NewSettings(http2.SETTINGS_NO, 0, http2.FLAG_NO).GetWire())
	...
	client.Send(http2.NewHeaders(headers, &client.Table, 1, http2.FLAG_END_HEADERS, 0, 0, false, 0).GetWire())
	time.Sleep(time.Second)
	client.Send(http2.NewGoAway(1, http2.NO_ERROR, "DEBUG string!!").GetWire())
}
